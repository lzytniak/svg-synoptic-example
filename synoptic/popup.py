...
from taurus.qt.qtgui.button import TaurusCommandButton, taurusbutton
...
class ResetCommandsWidgetPopup(CommandsWidget):
    """
    Popup variant of CommandsWidget.
    It moves to mouse on start and it closes on unfocus or after successful command.
    On show it hides irrelevant buttons.
    ...
    def doReset(self):
        print ('Resetting PLC'), self.resetAttr
    ...
    def setModel(self,model):
        ...
        try:
            aux = model.split('/')
            attrName = '/B_'+aux[0]+'_'+aux[1]+'_RESET_C'
            self.resetAttr = PyTango.AttributeProxy(model +attrName)
        except Exception:
            pass
